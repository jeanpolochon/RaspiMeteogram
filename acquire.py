...
val = bus.read_byte_data(0xBE, 0x30)
H0 = val / 2

# Read data back from 0x31, 1 byte
val = bus.read_byte_data(0xBE, 0x31)
H1 = val /2

# Read data back from 0x36, 2 bytes
val0 = bus.read_byte_data(0xBE, 0x36)
val1 = bus.read_byte_data(0xBE, 0x37)
H2 = ((val1 & 0xFF) * 256) + (val0 & 0xFF)

# Read data back from 0x3A, 2 bytes
val0 = bus.read_byte_data(0xBE, 0x3A)
val1 = bus.read_byte_data(0xBE, 0x3B)
H3 = ((val1 & 0xFF) * 256) + (val0 & 0xFF)

# Temperature Calibration values
# Read data back from 0x32, 1 byte
T0 = bus.read_byte_data(0xBE, 0x32)
T0 = (T0 & 0xFF)

# Read data back from 0x32, 1 byte
T1 = bus.read_byte_data(0xBE, 0x33)
T1 = (T1 & 0xFF)

# Read data back from 0x35, 1 byte
raw = bus.read_byte_data(0xBE, 0x35)
raw = (raw & 0x0F)
...
val0 = bus.read_byte_data(0xBE, 0x3C)
val1 = bus.read_byte_data(0xBE, 0x3D)
T2 = ((val1 & 0xFF) * 256) + (val0 & 0xFF)

# Read data back from 0x3E, 2 bytes
val0 = bus.read_byte_data(0xBE, 0x3E)
val1 = bus.read_byte_data(0xBE, 0x3F)
T3 = ((val1 & 0xFF) * 256) + (val0 & 0xFF)


# Select control register2, 0x21
#		0x01	One shot conversion
bus.write_byte_data(0xBE, 0x21, 0x01)

#While data is not ready, wait
while ((bus.read_byte_data(0xBE, 0x27) & 0x03) != 3):
	time.sleep(0.1)

# Read data back from 0x28 with command register 0x80, 4 bytes
# humidity msb, humidity lsb, temp msb, temp lsb
data = bus.read_i2c_block_data(0xBE, 0x28 | 0x80, 4)
...
bus.write_byte_data(0xBA, 0x21, 0x01)

while ((bus.read_byte_data(0xBA, 0x27) & 0x03) != 3):
	time.sleep(0.1)

# Read data back from 0x28 with Command register, 0x80
# 3 bytes, Pressure LSB first
data = bus.read_i2c_block_data(0xBA, 0x28 | 0x80, 3)
